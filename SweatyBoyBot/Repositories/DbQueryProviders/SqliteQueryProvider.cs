...
namespace SweatyBoyBot.Repositories.DbQueryProviders
{
	public class SqliteQueryProvider : IDbQueryProvider
	{
		...
		public Tuple<string, IEnumerable<Tuple<string, object>>> SaveOrUpdateWorkItemsOptions(IEnumerable<WorkItem> items)
		{
			...
			var newWorkItems = workItems.Where(e => e.Id == 0L).ToList();
			var existingWorkItems = workItems.Where(e => e.Id != 0L).ToList();
			...
			if (existingWorkItems.Any())
			{
				...
				{
					"INSERT INTO [workitem] ([Id], [NextRun], [Frequency], [ChannelId], [Content]) VALUES",
					string.Join(',', existingWorkItems.Select((e, i) => $"(@id{i}, @nr{i}, @fr{i}, @chid{i}, @cnt{i})")),
					"ON CONFLICT([Id]) DO UPDATE SET [NextRun] = excluded.[NextRun], [Frequency] = excluded.[Frequency], [ChannelId] = excluded.[ChannelId], [Content] = excluded.[Content];"
				});
			}
			...
			if (newWorkItems.Any())
			{
				queryLines.AddRange(new[] 
				{
					"INSERT INTO [workitem] ([NextRun], [Frequency], [ChannelId], [Content]) VALUES",
					string.Join(',', newWorkItems.Select((e, i) => $"(@newnr{i}, @newfr{i}, @newchid{i}, @newcnt{i})"))
				});
			}

			var parameters = existingWorkItems.SelectMany((e, i) => new[]
			{
			...
			}).Concat(newWorkItems.SelectMany((e, i) =>	new[] 
			{
				Tuple.Create($"@newnr{i}", e.NextRun.ToSweatyString() as object),
				Tuple.Create($"@newfr{i}", e.Frequency as object),
				Tuple.Create($"@newchid{i}", e.ChannelId as object),
				Tuple.Create($"@newcnt{i}", e.Content as object) 
			}));
